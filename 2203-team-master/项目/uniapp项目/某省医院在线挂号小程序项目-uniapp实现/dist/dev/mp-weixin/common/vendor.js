...
const PublicInstanceProxyHandlers = {
  get({ _: instance }, key) {
    ...
    } else if ({}.NODE_ENV !== "production" && currentRenderingInstance && (!isString(key) || key.indexOf("__v") !== 0)) {
  ...
  set({ _: instance }, key, value) {
    ...
    if (key[0] === "$" && key.slice(1) in instance) {
      ({}).NODE_ENV !== "production" && warn$1(`Attempting to mutate public property "${key}". Properties starting with $ are reserved and readonly.`);
      return false;
    } else {
...
function mergeOptions(to, from, strats, asMixin = false) {
  ...
  for (const key in from) {
    if (asMixin && key === "expose") {
      ({}).NODE_ENV !== "production" && warn$1(`"expose" option is ignored when declared in mixins or extends. It should only be declared in the base component itself.`);
    } else {
...
function updateProps(instance, rawProps, rawPrevProps, optimized) {
  ...
  if (!({}.NODE_ENV !== "production" && isInHmrContext(instance)) && (optimized || patchFlag > 0) && !(patchFlag & 16)) {
    if (patchFlag & 8) {
...
function patch(instance, data, oldData) {
  ...
  if (mpType === "page" || mpType === "component") {
    ...
    if (Object.keys(diffData).length) {
      if ({}.UNI_DEBUG) {
        console.log("[" + +new Date() + "][" + (mpInstance.is || mpInstance.route) + "][" + instance.uid + "][\u8017\u65F6" + (Date.now() - start) + "]\u5DEE\u91CF\u66F4\u65B0", JSON.stringify(diffData));
      }
...
function componentUpdateScopedSlotsFn() {
  ...
  if (Object.keys(diffData).length) {
    if ({}.UNI_DEBUG) {
      console.log("[" + +new Date() + "][" + (mpInstance.is || mpInstance.route) + "][" + this.uid + "][\u8017\u65F6" + (Date.now() - start) + "]\u4F5C\u7528\u57DF\u63D2\u69FD\u5DEE\u91CF\u66F4\u65B0", JSON.stringify(diffData));
    }
    mpInstance.setData(diffData);
  }
}
...
function useCssVars(getter) {
  ...
  if (!instance) {
    ({}).NODE_ENV !== "production" && warn$1(`useCssVars is called without current active component instance.`);
    return;
  }
...
const piniaSymbol = {}.NODE_ENV !== "production" ? Symbol("pinia") : Symbol();
...
const skipHydrateSymbol = {}.NODE_ENV !== "production" ? Symbol("pinia:skipHydration") : Symbol();
...
function createOptionsStore(id, options, pinia, hot) {
  ...
  function setup() {
    ...
    const localState = {}.NODE_ENV !== "production" && hot ? toRefs(ref(state ? state() : {}).value) : toRefs(pinia.state.value[id]);
...
function createSetupStore($id, setup, options = {}, pinia, hot, isOptionsStore) {
  ...
  const $reset = {}.NODE_ENV !== "production" ? () => {
...
function defineStore(idOrOptions, setup, setupOptions) {
  ...
  function useStore(pinia, hot) {
    ...
    pinia = ({}.NODE_ENV === "test" && activePinia && activePinia._testing ? null : pinia) || currentInstance2 && inject(piniaSymbol, null);
    if (pinia)
      setActivePinia(pinia);
...
This will fail in production.`);
    ...
    if ({}.NODE_ENV !== "production" && IS_CLIENT && currentInstance2 && currentInstance2.proxy && !hot) {
