    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <image-upload ref="myStaffPhoto" :limit="6" />
        </el-form-item>
...
export default {
  ...
  methods: {
    // 后端staffPhoto 始终存的是 字符串
    // 前端需要的是数组 [{}]
    async getPersonalDetail() {
      this.formData = await getPersonalDetail(this.userId) // 获取员工数据
      if (this.formData.staffPhoto) {
        // 6张图片- 后端数据库只是存的一个字符串
        this.$refs.myStaffPhoto.fileList = this.formData.staffPhoto.split(',').map(item => ({ url: item, status: 'success' }))
      }
    },
    async savePersonal() {
      const fileList = this.$refs.myStaffPhoto.fileList
      if (fileList.some(item => item.status !== 'success')) {
        this.$message.warning('您还有没上传完毕的图片, 请上传完后再保存')
        return
      }
      // [{ url: "abc" }] => ["abc", "efg"] => join => "abc,efg"
      await updatePersonal({ ...this.formData, id: this.userId, staffPhoto: fileList.map(item => item.url).join(',') })
    ...
    //  调用父组件
      ...
      // [{ url: '' }, {}]
      await saveUserDetailById({ ...this.userInfo, staffPhoto: fileList.map(item => item.url).join(',') })
      this.$message.success('保存成功')
    },
    async getUserDetailById() {
      ...
      if (this.userInfo.staffPhoto) {
        // 如果图片存在
        this.$refs.staffPhoto.fileList = [{ url: this.userInfo.staffPhoto, status: 'success' }]
      }
    }
  }
}
