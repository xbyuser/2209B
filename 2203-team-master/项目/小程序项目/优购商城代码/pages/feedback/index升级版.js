  ...
  handleFormSubmit() {
    ...
    if (chooseImgs.length != 0) {
      chooseImgs.forEach((v, i) => {
        wx.uploadFile({
          url: 'https://lianghj.top:8888/api/private/v1/upload', // 上传接口，用咱那个后台的接口测试
          // 图片上传组件的headers请求头对象
          header: {
          ...
          success: (result) => {
            ...
            // 把图片上传成功后返回的外网的链接存起来,连同文本内容一起在次请求接口给后台存起来。!!! 注意因为没给接口,此处就省略了，工作中会用到
            // let {url} = result.data;
            // console.log('上传的每个图片',url);
